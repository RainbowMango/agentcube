...
| :--- | :--- |
| `cmd/` | Main entry points for all AgentCube binaries (`workload-manager`, `router`, `picod`). |
...
| `pkg/picod/` | Implementation of the agent daemon (PicoD) that runs inside sandboxes. |
...
Located in `pkg/picod/`, this is the client-facing daemon (PicoD) that runs inside the sandbox container. It provides the execution environment for user scripts and commands.
